<?php
/*
 * All Back-End Helper Functions for Seil Theme
 * Author & Copyright: VictorThemes
 * URL: http://themeforest.net/user/VictorThemes
 */

/* Validate px entered in field */
if( ! function_exists( 'seil_check_px' ) ) {
  function seil_check_px( $num ) {
    return ( is_numeric( $num ) ) ? $num . 'px' : $num;
  }
}

/* Escape Strings */
if( ! function_exists( 'seil_vt_esc_string' ) ) {
  function seil_vt_esc_string( $num ) {
    return preg_replace('/\D/', '', $num);
  }
}

/* Escape Numbers */
if( ! function_exists( 'seil_vt_esc_number' ) ) {
  function seil_vt_esc_number( $num ) {
    return preg_replace('/[^a-zA-Z]/', '', $num);
  }
}

/* Compress CSS */
if ( ! function_exists( 'seil_compress_css_lines' ) ) {
  function seil_compress_css_lines( $css ) {
...
if( ! function_exists( 'seil_vt_upload_mimes' ) ) {
  function seil_vt_upload_mimes( $mimes ) {
  ...
  add_filter( 'upload_mimes', 'seil_vt_upload_mimes' );
}

/* Custom WordPress admin login logo */
if( ! function_exists( 'seil_theme_login_logo' ) ) {
  function seil_theme_login_logo() {
    $login_logo = cs_get_option('brand_logo_wp');
    if($login_logo) {
      $login_logo_url = wp_get_attachment_url($login_logo);
    } else {
      $login_logo_url = SEIL_IMAGES . '/logo.png';
    }
    if($login_logo) {
    echo "
      <style>
  ...
  add_action('login_head', 'seil_theme_login_logo');
}

/* WordPress admin login logo link */
if( ! function_exists( 'seil_login_url' ) ) {
  function seil_login_url() {
    return site_url();
  }
  add_filter( 'login_headerurl', 'seil_login_url', 10, 4 );
}

/* WordPress admin login logo link */
if( ! function_exists( 'seil_login_title' ) ) {
  function seil_login_title() {
    return get_bloginfo('name');
  }
  add_filter('login_headertitle', 'seil_login_title');
}
...
if ( ! function_exists( 'seil_tinymce_btns_font' ) ) {
  function seil_tinymce_btns_font( $buttons ) {
    array_unshift( $buttons, 'fontselect' ); // Add Font Select
    array_unshift( $buttons, 'fontsizeselect' ); // Add Font Size Select
    return $buttons;
  }
  add_filter( 'mce_buttons_2', 'seil_tinymce_btns_font' );
}

// Customize mce editor font sizes
if ( ! function_exists( 'seil_tinymce_sizes' ) ) {
  function seil_tinymce_sizes( $initArray ){
    $initArray['fontsize_formats'] = "9px 10px 12px 13px 14px 16px 18px 21px 24px 28px 32px 36px";
    return $initArray;
  }
  add_filter( 'tiny_mce_before_init', 'seil_tinymce_sizes' );
}

// Customize mce editor font family
if ( ! function_exists( 'seil_tinmymce_family' ) ) {
  function seil_tinmymce_family( $initArray ) {
  ...
  add_filter( 'tiny_mce_before_init', 'seil_tinmymce_family' );
}

/* HEX to RGB */
if( ! function_exists( 'seil_vt_hex2rgb' ) ) {
  function seil_vt_hex2rgb( $hexcolor, $opacity = 1 ) {
...
if( ! function_exists( 'seil_vt_yoast_metabox' ) ) {
  function seil_vt_yoast_metabox() {
    return 'low';
  }
  add_filter( 'wpseo_metabox_prio', 'seil_vt_yoast_metabox' );
}
...
if ( is_admin() ) {
  ...
  if( ! function_exists( 'seil_remove_rev_slider_meta_boxes' ) ) {
    function seil_remove_rev_slider_meta_boxes() {
    ...
    add_action( 'do_meta_boxes', 'seil_remove_rev_slider_meta_boxes' );
  }
