 ...
 */
if ( ! function_exists( 'seil_vt_get_current_theme_update' ) ) {
  function seil_vt_get_current_theme_update( $updates ) {

    if ( isset( $updates->checked ) ) {

      if ( ! class_exists( 'Envato_Protected_API' ) ) {
        require_once( SEIL_FRAMEWORK .'/plugins/theme-updater/class-envato-protected-api.php' );
      }
  ...
  add_filter( 'pre_set_site_transient_update_themes', 'seil_vt_get_current_theme_update' );
}

if ( ! function_exists( 'seil_vt_admin_head' ) ) {
  function seil_vt_admin_head() {
  ...
  add_action( 'admin_head', 'seil_vt_admin_head' );
}